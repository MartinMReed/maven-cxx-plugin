...
public class CommandLineService extends org.hardisonbrewing.maven.core.cli.CommandLineService {
    ...
    public static void addTabletEnvVars( Commandline commandLine ) {

        String blackberryTabletHome = PropertiesService.getProperty( PropertiesService.BLACKBERRY_TABLET_HOME );

        if ( blackberryTabletHome == null ) {
            return;
        }

        StringBuffer qnxHostBinDirPath = new StringBuffer();
        qnxHostBinDirPath.append( blackberryTabletHome );
